    ...
    async function fetchProducts() {
      try {
        ...
        if (data.status === 'success') {
          // Filter only featured products (is_featured === 1)
          const featuredProducts = data.data.filter((product: Product) => product.is_featured === 1)
          setProducts(featuredProducts)
          setHasFeaturedProducts(featuredProducts.length > 0)
        }
      } catch (error) {
...
const handleQuickAddToCart = (e: React.MouseEvent, product: Product) => {
  ...
  if (!loading && !hasFeaturedProducts) {
    return null
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="pt-32 pb-16 bg-gradient-to-br from-amber-50 via-teal-50 to-cyan-50 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 text-center relative z-10">
          <div className="inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm px-6 py-3 rounded-full mb-6 shadow-lg">
            <span className="text-sm font-medium text-gray-700">Handpicked Excellence</span>
          </div>
          
          <h1 className="text-5xl md:text-6xl font-light text-gray-800 mb-6 tracking-tight">
            Featured <span className="font-semibold text-transparent bg-clip-text bg-gradient-to-r from-teal-600 to-cyan-600">Collection</span>
          </h1>
          
          <p className="text-gray-600 text-lg max-w-2xl mx-auto leading-relaxed">
            Discover our handpicked selection of premium skincare products, carefully curated for exceptional results and luxurious experiences.
          </p>
          
          <div className="mt-8 flex items-center justify-center gap-8">
            <div className="text-center">
              <p className="text-3xl font-bold text-teal-600">{products.length}</p>
              <p className="text-sm text-gray-600 mt-1">Featured Products</p>
            </div>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-6">
          <div className="mb-12">
            <h2 className="text-lg font-medium text-gray-800 mb-6">Filter Featured Products</h2>
          ...
          {/* ENHANCED PRODUCT GRID WITH ZOOM EFFECT */}
            ...
            {filteredProducts.map((product) => (
              <div key={product.id} className="group cursor-pointer" onClick={() => openProductModal(product)}>
                <div className="relative overflow-hidden rounded-2xl mb-4 aspect-square bg-gray-50">
                  ...
                  <div className="absolute top-3 left-3 bg-gradient-to-r from-amber-500 to-amber-600 text-white px-3 py-1 rounded-full text-xs font-medium shadow-lg flex items-center gap-1">
                    Featured
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div className="relative aspect-square rounded-2xl overflow-hidden bg-gray-50">
                  ...
                  {selectedProduct.is_featured === 1 && (
                    <div className="absolute top-4 left-4 bg-gradient-to-r from-amber-500 to-amber-600 text-white px-4 py-2 rounded-full text-sm font-medium shadow-lg flex items-center gap-2">
                ...
                <div>
                  <div className="mb-4">
                    <div className="flex items-center gap-2 mb-3">
                      <span className="inline-block px-4 py-1 rounded-full text-sm bg-teal-50 text-teal-700">
                        {selectedProduct.category.name}
                      </span>
                      {selectedProduct.is_featured === 1 && (
                        <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm bg-amber-50 text-amber-700 border border-amber-200">
                  ...
                  <div className="space-y-3 mb-8 p-4 bg-gray-50 rounded-xl">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-amber-600">
                        {selectedProduct.is_featured === 1 ? 'Featured Product' : 'Standard Product'}
                      </span>
                    </div>
                  </div>
                </div>
